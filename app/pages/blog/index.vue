...
<template>
  ...
  <div class="mt-10 px-5 relative md:pt-20">
    ...
    <div v-if="posts?.length" class="mx-auto mt-10 grid max-w-2xl grid-cols-1 gap-x-8 gap-y-16 border-t border-gray-200 dark:border-muted border-dashed pt-10 sm:mt-16 sm:pt-16 lg:mx-0 lg:max-w-none lg:grid-cols-3 xl:grid-cols-4">
      <article v-for="(post, index) in posts" :key="post.id" class="flex max-w-xl flex-col items-start justify-between border border-accented p-4 rounded-xl" v-motion-slide-visible-once-bottom :delay="100 * index">
        <NuxtImg height="10rem" :src="post.image" class="rounded-md w-full object-cover" alt="Cover image for this blog post" placeholder />
        ...
        <div class="group relative">
          <h3 class="mt-3 text-lg/6 font-bold group-hover:text-muted line-clamp-2">
